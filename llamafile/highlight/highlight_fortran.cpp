...
void
HighlightFortran::feed(std::string* r, std::string_view input)
{
    int c;
        ...
        case WORD:
            if (!isascii(c) || isalnum(c) || c == '_') {
                ...
                if (is_keyword_fortran(word_.data(), word_.size())) {
                    ...
                    *r += word_;
                    *r += HI_RESET;
                } else if (is_keyword_fortran_type(word_.data(),
                                                   word_.size())) {
                    *r += HI_TYPE;
                    *r += word_;
                    *r += HI_RESET;
                } else if (is_keyword_fortran_builtin(word_.data(),
                                                      word_.size())) {
                    *r += HI_BUILTIN;
                    *r += word_;
                    *r += HI_RESET;
...
void
HighlightFortran::flush(std::string* r)
{
    switch (t_) {
    case WORD:
        if (is_keyword_fortran(word_.data(), word_.size())) {
