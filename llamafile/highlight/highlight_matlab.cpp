...
void
HighlightMatlab::feed(std::string* r, std::string_view input)
{
    int c;
        ...
        case WORD:
            if (!isascii(c) || isalnum(c) || c == '_') {
                ...
                if (is_keyword_matlab(word_.data(), word_.size())) {
                    ...
                    *r += word_;
                    *r += HI_RESET;
                } else if (is_keyword_matlab_builtin(word_.data(),
                                                     word_.size())) {
                    *r += HI_BUILTIN;
                    *r += word_;
                    *r += HI_RESET;
                } else if (is_keyword_matlab_constant(word_.data(),
                                                      word_.size())) {
                    *r += HI_CONSTANT;
                    *r += word_;
                    *r += HI_RESET;
...
void
HighlightMatlab::flush(std::string* r)
{
    switch (t_) {
    case WORD:
        if (is_keyword_matlab(word_.data(), word_.size())) {
