...
void
HighlightScala::feed(std::string* r, std::string_view input)
{
    int c;
    for (size_t i = 0; i < input.size(); ++i) {
        ...
        case WORD:
            if (!isascii(c) || isalnum(c) || c == '_') {
                ...
                if (is_keyword_scala(word_.data(), word_.size())) {
                    *r += HI_KEYWORD;
                    *r += word_;
                    *r += HI_RESET;
...
void
HighlightScala::flush(std::string* r)
{
    switch (t_) {
    case WORD:
        if (is_keyword_scala(word_.data(), word_.size())) {
